...
namespace CafofoStudio
{
    ...
    public class SoundElement
    {
        ...
        private float elapsedTimeSinceSampleUpdate = 0f;
        private float elapsedTimeSinceLoopUpdate = 0f;

        private const float UPDATE_SAMPLE_INTERVAL = 0.1f;
        private const float UPDATE_LOOP_INTERVAL = 1f;
        ...
        {
            elapsedTimeSinceSampleUpdate += Time.deltaTime;

            if (elapsedTimeSinceSampleUpdate / UPDATE_SAMPLE_INTERVAL > 1)
            {
                foreach (SoundSubElementSample s in sampleSubElements)
                {
                    s.UpdateSampleTimer(intensity, volumeMultiplier, elapsedTimeSinceSampleUpdate);
                }

                elapsedTimeSinceSampleUpdate = 0f;
            }

            elapsedTimeSinceLoopUpdate += Time.deltaTime;
            
            if (elapsedTimeSinceLoopUpdate / UPDATE_LOOP_INTERVAL > 1)
            {
                foreach (SoundSubElementLoop s in loopSubElements)
                {
                    s.PerformCleanupPass();
                }

                elapsedTimeSinceLoopUpdate = 0f;
            }
        }
